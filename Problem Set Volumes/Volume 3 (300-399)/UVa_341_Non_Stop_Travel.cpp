...
/*
	...
	To build using Visucal Studio 2008:
		cl -EHsc UVa_341_Non_Stop_Travel.cpp
...
void print_path(int start, int end, const vector<int>& parent_vertices)
{
	if (start != end) {
		print_path(start, parent_vertices[end], parent_vertices);
		cout << ' ' << end;
	}
	else
		cout << end;
...
int main()
{
	for (int case_nr = 1; ; case_nr++) {
		...
		if (!n)
			break;
		vector< vector<edge> > edges(n + 1); // edges[i] is the vector of edges from the i-th vertex
		for (int u = 1; u <= n; u++) {
			int i;
			cin >> i;
			while (i--) {
		...
		int start, end;
		cin >> start >> end;
		cout << "Case " << case_nr << ": Path = ";
		if (start == end)
			cout << start << ' ' << end << "; 0 second delay\n";
		else {
			vector<vertex_distance> distances(n + 1);
			vector<int> parent_vertices(n + 1);
			// apply Dijkstra's shortest path algorithm
			shortest_path(n + 1, start, end, edges, distances, parent_vertices);
			print_path(start, end, parent_vertices);
			cout << "; " << distances[end].distance << " second delay\n";
		}
	}
	return 0;
}
