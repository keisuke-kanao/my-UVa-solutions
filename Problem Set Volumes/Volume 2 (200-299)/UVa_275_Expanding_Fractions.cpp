...
/*
	...
	To build using Visual Studio 2008:
		cl -EHsc -O2 UVa_275_Expanding_Fractions.cpp
...
int main()
{
	while (true) {
		...
		while (true) {
			...
			numerator -= quotient * denominator;
			pair<map<int, size_t>::iterator, bool> result = remainders.insert(make_pair(numerator, quotients.size()));
			if (!result.second) {
		...
		if (nr_printed)
			cout << endl;
		if (nr_repeat)
			cout << "The last " << j - repeat << " digits repeat forever.\n\n";
		else
