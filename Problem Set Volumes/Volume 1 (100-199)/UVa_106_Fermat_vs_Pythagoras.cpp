...
/*
	...
	To build using Visual Studio 2008:
		cl -EHsc -O2 UVa_106_Fermat_vs_Pythagoras.cpp
*/
...
int calculate_pythagorean_triples(int n, int x, int y, int z, vector<bool>& triples)
{
	if (x > n || y > n || z > n)
		return 0;
#ifdef DEBUG
	cerr << x << ' ' << y << ' ' << z << endl;
#endif
	for (int nx = x, ny = y, nz = z; nx <= n && ny <= n && nz <= n; nx += x, ny += y, nz += z)
