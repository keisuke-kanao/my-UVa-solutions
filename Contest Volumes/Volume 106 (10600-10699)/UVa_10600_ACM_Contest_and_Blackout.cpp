...
/*
	...
	To build using Visucal Studio 2008:
		cl -EHsc -O2 UVa_10600_ACM_Contest_and_Blackout.cpp
*/
...
int mst_prim(int n, int start, const vector< vector<edge> >& graph, vector<int>& parents)
{
...
int second_best_mst_prim(int n, int start, const vector< vector<edge> >& graph, int ei, int ej)
{
	vector<vertex_distance> distances(n);
	for (int i = 0; i < n; i++)
		distances[i] = vertex_distance(i, ((i != start) ? numeric_limits<int>::max() : 0));
...
int main()
{
	...
	while (t--) {
		...
		int start, mst_distance = numeric_limits<int>::max();
		vector<int> parents(n);
		for (start = 0; start < n; start++)
			if ((mst_distance = mst_prim(n, start, graph, parents)) != numeric_limits<int>::max())
				break;
		int second_mst_distance = numeric_limits<int>::max();
		for (int i = 0; i < n; i++)
			if (i != start)
				second_mst_distance = min(second_mst_distance, second_best_mst_prim(n, start, graph, i, parents[i]));
