...
int mst_prim(int n, int start, const vector< vector<edge> >& graph)
{
	int i;
	...
	pqueue_free(queue);
	for (i = 0; i < n; i++)
		if (distances[i].distance == numeric_limits<int>::max()) {
			mst_distance = -1; break;
		}
	return mst_distance;
}
...
int main()
{
	while (true) {
		...
		for (i = 0; i < s; i++) {
			string s;
			cin >> s;
			stations.insert(make_pair(s, i));
		}
		vector< vector<edge> > graph(s);
		for (i = 0; i < c; i++) {
			string s, t;
			int w;
			cin >> s >> t >> w;
			int u = stations[s], v = stations[t];
		...
		cin >> ss;
		int su = stations[ss];
		// calculate the minimum spanning tree and see if all the vertices are covered
		int mst_distance = mst_prim(s, su, graph);
		if (mst_distance != -1)
